...
// Transfer amount from counter manager to admin
if (isset($_GET['availAmount']) and isset($_GET['amount']) and isset($_GET['mode']) and isset($_GET['managerId_counter']) and isset($_GET['counterId'])) {
    include "../include/connect.php";
    
    $availAmount = (int)$_GET['availAmount'];
    $amount = (int)$_GET['amount'];
    $managerId = (int)$_GET['managerId_counter'];
    $counterId = (int)$_GET['counterId'];
    $mode = (int)$_GET['mode'];

    $sql = "INSERT INTO `transfertoownercounter`(`senderid`, `amount`, `mode`, `counter`) VALUES ( ? , ? , ? , ? )";
    $stmt = $conn->prepare($sql);

    if ($stmt) {
        $stmt->bind_param("iiii", $managerId, $amount, $mode, $counterId);
        if ($stmt->execute()) {
            $res = [
                'status' => true,
                'msg' => 'EveryThing is fine'
...
if (isset($_POST['submitCounter'])) {
    ...
    $sql = "INSERT INTO `counter`(`customerName`, `created_by`, `date`) VALUES (? , ? , ?)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("sis", $customerName, $createdBy, $dateofGaadi);
        if ($stmt->execute()) {
            // I want to return id just created with above insert query
            $lastInsertId = $conn->insert_id;
...
if (isset($_GET["payamountsubmit"])) {
    ...
    $counterid = (int)$_GET["counterid"];
    $type = (int)$_GET["payamounttype"];

    $sql = "INSERT INTO `amountpaidcounters`(`counter`, `amount`, `type`) VALUES ( ? , ? , ?)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("iii", $counterid, $amountPaid, $type);
        if ($stmt->execute()) {
            // Update total amountpaid in counter table
            $sqlUpdate = "UPDATE `counter` SET `amountpaid` = (SELECT COALESCE(SUM(amount), 0) FROM amountpaidcounters WHERE counter = ?) WHERE `id` = ?";
            $stmtUpdate = $conn->prepare($sqlUpdate);
            if($stmtUpdate) {
                $stmtUpdate->bind_param("ii", $counterid, $counterid);
                $stmtUpdate->execute();
            }
...
if (isset($_POST['changePasswordSubmit'])) {
    ...
    if($newPassword !== $confirmPassword) {
        header('location:changePassword.php?error=Passwords do not match');
        exit();
    }

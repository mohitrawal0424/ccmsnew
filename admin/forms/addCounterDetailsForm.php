    ...
    <div id="addCounterDetailsForm" class="p-4 w-full max-w-2xl md:h-auto">
        ...
        <div class="p-4 bg-gray-700 rounded-lg shadow-2xl sm:p-5 border-2 border-black">
            ...
            <form action="./queries.php" method="GET">
                <input type='hidden' id='counterid' name='counterid' value='<?php echo $recId; ?>'>
                <div class="grid gap-4 mb-4 sm:grid-cols-2 text-white">
                    <div class="bg-white rounded-md p-2 m-2" id="customerDetails">
                        <?php
                        include("../include/connect.php");
                        $sql = "SELECT c.*,c.customerName as recName FROM `counter` as c 
                            WHERE c.id = ?";
                        ...
                        if ($stmt) {
                            ...
                            if ($stmt->execute()) {
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    // Get total amount paid from amountpaidcounters table
                                    $sqlPaid = "SELECT COALESCE(SUM(amount), 0) as total FROM amountpaidcounters WHERE counter = ?";
                                    $stmtPaid = $conn->prepare($sqlPaid);
                                    $amountpaid = 0;
                                    $cashPaid = 0;
                                    $onlinePaid = 0;
                                    if($stmtPaid) {
                                        $stmtPaid->bind_param("i", $recId);
                                        if($stmtPaid->execute()) {
                                            $resultPaid = $stmtPaid->get_result();
                                            $amountpaid = $resultPaid->fetch_assoc()['total'];
                                        }
                                    }
                                    
                                    // Get cash and online breakdown
                                    $sqlCash = "SELECT COALESCE(SUM(amount), 0) as cash FROM amountpaidcounters WHERE counter = ? AND type = 1";
                                    $stmtCash = $conn->prepare($sqlCash);
                                    if($stmtCash) {
                                        $stmtCash->bind_param("i", $recId);
                                        if($stmtCash->execute()) {
                                            $cashPaid = $stmtCash->get_result()->fetch_assoc()['cash'];
                                        }
                                    }
                                    
                                    $sqlOnline = "SELECT COALESCE(SUM(amount), 0) as online FROM amountpaidcounters WHERE counter = ? AND type = 2";
                                    $stmtOnline = $conn->prepare($sqlOnline);
                                    if($stmtOnline) {
                                        $stmtOnline->bind_param("i", $recId);
                                        if($stmtOnline->execute()) {
                                            $onlinePaid = $stmtOnline->get_result()->fetch_assoc()['online'];
                                        }
                                    }
                                                    ...
                                                    ?>
                                                        ...
                                                        ?>
                                                            <tr class="bg-white border border-gray-800">
                                                                ...
                                                                <div class="flex">
                                                                <!-- hidden fields  -->
                          <input class="availableStoke" type="hidden" name="availableStoke" value="<?php echo $row2["nos"]; ?>">
                          <input class="tableId" type="hidden" name="tableId" value="<?php echo $row2["gdid"]; ?>">
                          <input type="hidden" name="itemtype" class="itemtype" value="<?php echo $row2['itemtype'] ?>">
                          <input type="hidden" name="itemid" class="itemid" value="<?php echo $row2['itemid'] ?>">
                          <!-- hidden fields  -->
                                                                <input type="number" name="ReturnQuantity" class="ReturnQuantity border border-black rounded" placeholder="Enter return qty">
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                            ...
                                                            <!-- return peti input and button  -->
                                                                ...
                                                                <div class="flex">
                                                                <!-- hidden fields  -->
                          <input class="availableStoke" type="hidden" name="availableStoke" value="<?php echo $row4["nos"]; ?>">
                          <input class="tableId" type="hidden" name="tableId" value="<?php echo $row4["id"]; ?>">
                          <input type="hidden" name="itemtype" class="itemtype" value="<?php echo $row4['itemtype'] ?>">
                          <input type="hidden" name="itemid" class="itemid" value="<?php echo $row4['petiID'] ?>">
                          <!-- hidden fields  -->
                                                                <input type="number" name="ReturnQuantity" class="ReturnQuantity border border-black rounded" placeholder="Enter return qty">
                ...
                <div class="sm:col-span-2 text-md rounded-lg bg-white text-gray-700 px-2 py-1 mb-3">
                    ...
                    <select name="payamounttype" class="bg-gray-200 border-blue-600 border-2 rounded text-black p-1">
                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 mb-2">
                        <tbody>
                            ...
                            <tr class="bg-white border border-gray-800">
                                ...
                                <td class="px-3 py-2 text-gray-700">
                                    <?php
                                    echo $amountpaid . " (Cash: " . $cashPaid . ", Online: " . $onlinePaid . ")";
                                    ?>
                                </td>
                            </tr>
                            <tr class="bg-white border border-gray-800">

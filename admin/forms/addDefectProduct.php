...
<div class="p-4 bg-gray-700 rounded-lg shadow-2xl sm:p-5 border-2 border-black">
           ...
           <div id="errorDiv" class='text-red-400  p-2 mt-2 rounded-md text-xs hidden'> Can't be Empty</div>
            ...
            <div class="grid gap-4 mb-4 sm:grid-cols-2 text-white">
                                ...
                                if ($stmt->execute()) {
                                    ...
                                    if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                echo '<option value="'.$row['id'].'">'.$row['name'] .' ' . convertToLitre($row['size']) .'</option>';
                                            }
                                    }
                                } else {
                ...
                <button name="defectproductsubmit" id="defectproductsubmit" type="submit" class="text-white inline-flex items-center bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-10 py-2.5 text-center">
                    Add
                </button>
            </form>
        </div>
